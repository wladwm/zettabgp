...
impl BmpMessageRouteMonitoring {
    pub fn decode_from(
        buf: &[u8]
    ) -> Result<(BmpMessageRouteMonitoring,usize), BgpError> {
        if buf.len()<62 {
        ...
        if msgt.0!=BgpMessageType::Update {
            return Err(BgpError::static_str("Invalid BGP message type for BmpMessageRouteMonitoring"))
        }
        let mut upd = BgpUpdateMessage::new();
        upd.decode_from(sesspars, &buf[pos..pos+msgt.1])?;
        pos=pos+msgt.1;
        Ok((BmpMessageRouteMonitoring{peer:pm.0,update:upd},pos))
    }
}
