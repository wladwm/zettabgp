...
impl BmpMessageHeader {
    pub fn decode_from(
        buf: &[u8]
    ) -> Result<(BmpMessageHeader,usize), BgpError> {
        if buf.len()<5 {
...
impl BmpMessage {
    pub fn decode_from(
        buf: &[u8]
    ) -> Result<BmpMessage, BgpError> {
        let msgtype=buf[0];
        match msgtype {
          0 => Ok(BmpMessage::RouteMonitoring(BmpMessageRouteMonitoring::decode_from(&buf[1..])?.0)),
          1 => Ok(BmpMessage::StatisticsReport),
          2 => Ok(BmpMessage::PeerDownNotification),
          3 => Ok(BmpMessage::PeerUpNotification(BmpMessagePeerUp::decode_from(&buf[1..])?.0)),
          4 => Ok(BmpMessage::Initiation(BmpMessageInitiation::decode_from(&buf[1..])?.0)),
          5 => Ok(BmpMessage::Termination(BmpMessageTermination::decode_from(&buf[1..])?.0)),
          6 => Ok(BmpMessage::RouteMirroring),
          _ => {
            Err(BgpError::static_str("Invalid BMP message type"))
          }
        }
    }
}
