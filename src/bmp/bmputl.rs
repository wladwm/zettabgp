...
pub fn decode_bmp_addr_from(buf: &[u8]) -> Result<std::net::IpAddr, BgpError> {
    ...
    if buf[0]==0 && buf[1]==0 && buf[2]==0 && buf[3]==0 && buf[4]==0 && buf[5]==0 && buf[6]==0 && buf[7]==0 && buf[8]==0 && buf[9]==0 && buf[10]==0 && buf[11]==0 {
...
impl From<&BmpMessagePeerHeader> for BgpSessionParams {
    ...
    fn from(bmph:&BmpMessagePeerHeader) -> BgpSessionParams {
        BgpSessionParams::new(bmph.asnum, 0, bmph.peeraddress.into(), bmph.routerid, std::collections::HashSet::new())
    }
}
impl BmpMessagePeerHeader {
    pub fn decode_from(
        buf: &[u8]
    ) -> Result<(BmpMessagePeerHeader,usize), BgpError> {
        if buf.len()<42 {
            return Err(BgpError::insufficient_buffer_size());
        }
        Ok((BmpMessagePeerHeader{
            peertype: buf[0],
            flags: buf[1],
